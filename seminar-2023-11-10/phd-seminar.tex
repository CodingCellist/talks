  ...
  \begin{itemize}
    \item<1-> Compression can be proven to be correctly implemented
      \begin{itemize}
        ...
        \item<4-> Now the compiler can help us implement them and check that we
                  have done so correctly
      \end{itemize}
    \item<5-> QuickCheck can simulate network behaviour
      \begin{itemize}
        \item<6-> Begin at an arbitrary point
        \item<7-> Simulate chance of failure
        \item<8-> Potentially with arbitrary parameters for each run
      \end{itemize}
    \item<9-> Some boolean tests could be added for the network implementation
      \begin{itemize}
        \item<10-> The simulated behaviour is well-behaved\dots
        \item<11-> {\dots} is the actual behaviour?
      \end{itemize}
  \end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item<2-> We do not need to trust we have modelled the problem correctly, we
              can test that!
    \item<3-> Can use as strong or weak verification as we want
    \item<4-> The datatypes used when verifying are the same ones used for the
              implementation
