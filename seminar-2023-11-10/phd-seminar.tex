...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item<6-> We often want to compose code
    \end{itemize}
  \end{itemize}
\end{frame}
  ...
  \begin{column}{0.7\framewidth}
    \begin{itemize}
      ...
      \item<4-> Wait for the receiver to acknowledge they are ready for the next
            packet
      \item<5-> Resend if no acknowledgement is received within a given timeout
    \end{itemize}
  \end{column}

  \begin{column}{0.3\framewidth}<6->
    \todo{Overlay annotations?}
    \begin{figure}
      \begin{tikzpicture}[> = stealth, thick]
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item<2-> Instead of ``competing'', combine the systems to work together
